...
export default class PostgresAdminRepository extends AdminRepository {
  ...
  async update(adminId, { adminName, email, passwordHash }) {
    ...
    // 2️⃣ Update admins table only if adminName is provided
    if (adminName) {
    ...
    // 3️⃣ Update users table for email/password only if provided
    const setParts = [];
    const values = [];
    let idx = 1;
    ...
    if (setParts.length > 0) {
      values.push(userId);
      await this.db.query(
        `UPDATE users SET ${setParts.join(", ")} WHERE user_id=$${idx}`,
        values
      );
    }
