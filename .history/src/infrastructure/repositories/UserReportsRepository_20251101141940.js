// src/infrastructure/repositories/UserReportsRepository.js
import db from "../db/postgres.js";
import UserReport from "../../domain/Entities/UserReport.js";

export default class UserReportsRepository {
  async getReportsByRole(role) {
    let query = "";
    if (role === "client") {
      query = `
        SELECT 
        ur.report_id,
        ur.reported_user_id,
        ur.reporter_user_id,
        ur.evidence_text,
        ur.evidence_image,
        ur.date_reported,
        u1.is_banned,
        COALESCE(c.client_name, 'Unknown Client') AS reported_user_name,
        COALESCE(c2.client_name, 'Unknown Reporter') AS reporter_user_name
    FROM user_reports ur
    JOIN users u1 ON ur.reported_user_id = u1.user_id
    LEFT JOIN clients c ON u1.user_id = c.user_id
    JOIN users u2 ON ur.reporter_user_id = u2.user_id
    LEFT JOIN clients c2 ON u2.user_id = c2.user_id
    WHERE u1.role = 'client';
      `;
    } else if (role === "clinic_owner") {
      query = `
        SELECT 
        ur.report_id,
        ur.reported_user_id,
        ur.reporter_user_id,
        ur.evidence_text,
        ur.evidence_image,
        ur.date_reported,
        u1.is_banned,
        COALESCE(cl.clinic_name, 'Unknown Clinic') AS reported_user_name,
        COALESCE(cl2.clinic_name, 'Unknown Reporter Clinic') AS reporter_user_name
        FROM user_reports ur
        JOIN users u1 ON ur.reported_user_id = u1.user_id
        LEFT JOIN clinics cl ON cl.owner_id = u1.user_id
        JOIN users u2 ON ur.reporter_user_id = u2.user_id
        LEFT JOIN clinics cl2 ON cl2.owner_id = u2.user_id
        WHERE u1.role = 'clinic_owner';
  `;
    } else if (role === "veterinarian") {
      query = `
        SELECT 
        ur.report_id,
        ur.reported_user_id,
        ur.reporter_user_id,
        ur.evidence_text,
        ur.evidence_image,
        ur.date_reported,
        u1.is_banned,
        COALESCE(v.name, 'Unknown Veterinarian') AS reported_user_name,
        COALESCE(v2.name, 'Unknown Reporter Veterinarian') AS reporter_user_name
        FROM user_reports ur
        JOIN users u1 ON ur.reported_user_id = u1.user_id
        LEFT JOIN veterinarians v ON v.user_id = u1.user_id
        JOIN users u2 ON ur.reporter_user_id = u2.user_id
        LEFT JOIN veterinarians v2 ON v.user_id = u2.user_id
        WHERE u1.role = 'veterinarian';
  `;
    }

    const { rows } = await db.query(query);
    return rows.map((r) => new UserReport(r));
  }

  async banUser(reported_user_id) {
    // 1️⃣ Get latest evidence for the user
    const evidenceQuery = `
      SELECT evidence_text, evidence_image
      FROM user_reports
      WHERE reported_user_id = $1
      ORDER BY date_reported DESC
      LIMIT 1;
    `;
    const evidenceResult = await db.query(evidenceQuery, [reported_user_id]);
    const evidence = evidenceResult.rows[0];

    if (!evidence) throw new Error("No report found for this user.");

    // 2️⃣ Ban user using evidence
    const updateQuery = `
      UPDATE users
      SET 
        is_banned = TRUE,
        ban_reason = $1,
        evidence_image = $2
      WHERE user_id = $3
      RETURNING *;
    `;
    const { rows } = await db.query(updateQuery, [
      evidence.evidence_text,
      evidence.evidence_image,
      reported_user_id,
    ]);

    return rows[0];
  }

  // ✅ Helper (for controller validation)
  async findUserById(user_id) {
    const { rows } = await db.query("SELECT * FROM users WHERE user_id = $1", [
      user_id,
    ]);
    return rows[0] || null;
  }
}
