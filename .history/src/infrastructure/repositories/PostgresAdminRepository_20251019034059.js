...
LEFT JOIN addresses ad ON a.address_id = ad.address_id  -- ✅ changed JOIN → LEFT JOIN
WHERE u.email = $1
LIMIT 1
    `;
  ...
  `;
    ...
    if (result.rows.length === 0) {
      console.log("⚠️ No admin found for ID:", adminId);
      return null;
    }

    const r = result.rows[0];
    console.log("✅ findById() raw result:", r);

    // Make sure 'password' exists
    if (!r.password) {
      console.error("❌ Missing password in findById() result!");
    }
    ...
    return {
      ...
      email: r.email,
      role: r.role,
      passwordHash: r.password, // ✅ fix here
      phoneNumber: r.phone_number,
    };
  }
