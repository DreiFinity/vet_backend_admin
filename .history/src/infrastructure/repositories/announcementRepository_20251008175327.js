...
export default class AnnouncementRepository extends IAnnouncementRepository {
  ...
  async create(announcement) {
    ...
    const values = [
      ...
      announcement.target_role_id, // <-- uses role_id
  ...
  // Get all announcements, optionally filtered by role
  async getAll(role_id) {
    let query = "SELECT * FROM announcements";
    let values = [];

    if (role_id && role_id !== "all") {
      query += " WHERE target_role_id = $1 OR target_role_id IS NULL";
      values = [role_id];
    }
  ...
  async update(id, data) {
    ...
    const values = [
      ...
      data.content,
      data.category,
      data.target_role_id, // <-- updated to role_id
      data.priority,
      data.status,
      data.start_datetime,
      data.end_datetime,
