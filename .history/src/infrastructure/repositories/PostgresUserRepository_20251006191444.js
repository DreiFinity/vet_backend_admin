...
export default class PostgresUserRepository extends IUserRepository {
  ...
  async getBannedUsers() {
    const { rows } = await pool.query(`
      SELECT u.user_id, u.email, c.client_name as fName, c.client_name as lName,
             c.phone as contact, c.bio as ban_reason, 0 as pets_registered
      FROM users u
      JOIN clients c ON u.user_id = c.user_id
  ...
  async unbanUser(userId) {
    const { rows } = await pool.query(
      `UPDATE users SET is_banned = false, ban_reason = NULL WHERE user_id = $1 RETURNING user_id, email, is_banned`,
      [userId]
    );
    return rows[0];
  }
