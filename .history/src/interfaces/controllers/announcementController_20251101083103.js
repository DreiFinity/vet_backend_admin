...
export const getPetOwnersAnnouncements = async (req, res) => {
  try {
    const query = `
      SELECT *
      FROM announcements
      WHERE 
        (target_role_id = 1 OR target_role_id IS NULL)
        AND status ILIKE 'Published'
        AND (
          start_datetime IS NULL OR start_datetime <= NOW()
        )
    ...
    console.error("❌ Error fetching pet owners announcements:", err);
    res
      .status(500)
      .json({ message: "Failed to fetch pet owners announcements" });
  }
};

export const getClinicsAnnouncements = async (req, res) => {
  try {
    const query = `
      SELECT *
      FROM announcements
      WHERE 
        (target_role_id = 2 OR target_role_id IS NULL)
        AND status ILIKE 'Published'
        AND (
          start_datetime IS NULL OR start_datetime <= NOW()
        )
    ...
    console.error("❌ Error fetching clinics announcements:", err);
    res.status(500).json({ message: "Failed to fetch clinics announcements" });
  }
};

export const getVetsAnnouncements = async (req, res) => {
  try {
    const query = `SELECT *
      FROM announcements
      WHERE 
        (target_role_id = 4 OR target_role_id IS NULL)
        AND status ILIKE 'Published'
        AND (
          start_datetime IS NULL OR start_datetime <= NOW()
        )
    ...
    console.error("❌ Error fetching veterinarian announcements:", err);
    res
      .status(500)
      .json({ message: "Failed to fetch veterinarian announcements" });
  }
};
